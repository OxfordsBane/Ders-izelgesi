...
def analyze_data(teachers, classes):
    ...
    classes_map = {c['Sınıf Adı']: c for c in classes}
    for t in teachers:
        ...
        if fixed_class:
            target_class = classes_map.get(fixed_class)
            if not target_class:
    ...
    if logic_errors:
        ...
        if st.button("🚀 Programı Oluştur"):
            with st.spinner("Esnek modda çözüm üretiliyor..."):
                ...
                # Sabit Sınıf (Hard)
                class_index_map = {c['Sınıf Adı']: i for i, c in enumerate(classes_list)}
                for t_idx, t in enumerate(teachers_list):
                    if t['Sabit Sınıf']:
                        fixed_c_idx = class_index_map.get(str(t['Sabit Sınıf']))
